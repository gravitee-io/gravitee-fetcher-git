...
Use an `https://` repository URL: credentials configured against a plain `http://` remote travel unencrypted, and the
gateway logs a warning to that effect on every fetch.
